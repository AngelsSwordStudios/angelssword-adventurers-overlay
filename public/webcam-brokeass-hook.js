// ═══════════════════════════════════════════════════
//  BrokeAss webcam loop — smile + open-mouth geometry
//  Takes over Start/Stop webcam buttons and injects
//  landmark ratios (same as BrokeAss tracker.py).
// ═══════════════════════════════════════════════════
...
(() => {
  ...
  function ensureWS() {
    ...
    try {
      ...
      ws.addEventListener('open', function () {
        console.log('[brokeass-webcam] WS connected');
      });
      ...
      console.warn('[brokeass-webcam] WS error', e);
    }
    return ws;
  }
  ...
  async function loadMP() {
    ...
    console.log('[brokeass-webcam] MediaPipe ready (BrokeAss geometry)');
  ...
  function processFrame() {
    ...
    if (hasLandmarks || hasBlend) {
      ...
      // Blendshapes kept for eyesClosed / frown fallback
      if (hasBlend) {
      ...
      // BrokeAss geometry → _smileRatio + _mouthOpenRatio (0–100, gain applied)
      if (hasLandmarks && window.AS_BrokeAss) {
        window.AS_BrokeAss.injectGeometryScores(blendShapeMap, result.faceLandmarks[0]);
      }
  ...
  function takeOver() {
    ...
    if (!btn) {
      console.warn('[brokeass-webcam] start button missing');
      return;
    }

    // Drop control.js listeners
    ...
    fresh.addEventListener('click', async function () {
      ...
      try {
        ...
        fresh.disabled = true;
        if (statusEl) {
          statusEl.textContent = 'Status: Loading MediaPipe (BrokeAss smile + open-mouth)...';
        ...
        if (statusEl) {
          statusEl.textContent = 'Status: Tracking ✓ BrokeAss geometry';
    ...
    console.log('[brokeass-webcam] Buttons taken over — smile + open-mouth use landmark geometry');
  }
